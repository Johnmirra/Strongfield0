...
public class Louncher : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        //Get target radius from bonus
        if(!chekBonusRadiusIsGet)
            if(GameMechanics.Mechanics.rlRadius)
            {
                chekBonusRadiusIsGet = true;
                targetRadius.radius *= GameMechanics.Mechanics.BonusLouncherRadius;
    ...
    void Fire()
    {
        if (timeToShut < 0)
        {
            ...
            source.Play();
            if (GameMechanics.Mechanics.louncherFR)
                timeToShut = GameMechanics.Mechanics.LouncherReloatTime - GameMechanics.Mechanics.BonusLouncherFR;
            else
                timeToShut = GameMechanics.Mechanics.LouncherReloatTime;
            ...
            myBullet.GetComponent<Rocket>().damage = damage;
